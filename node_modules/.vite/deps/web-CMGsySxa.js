...
import { Persistence, ProviderId } from "./@capacitor-firebase_authentication.js";
import { At as verifyBeforeUpdateEmail, C as TwitterAuthProvider, D as browserLocalPersistence, Dt as updateProfile, F as deleteUser, H as getRedirectResult, I as fetchSignInMethodsForEmail, J as linkWithCredential, L as getAdditionalUserInfo, M as connectAuthEmulator, N as createUserWithEmailAndPassword, R as getAuth, St as unlink, Tt as updatePassword, U as inMemoryPersistence, W as indexedDBLocalPersistence, X as linkWithPopup, Y as linkWithPhoneNumber, Z as linkWithRedirect, _t as signInWithEmailLink, bt as signInWithRedirect, ct as revokeAccessToken, d as OAuthCredential, dt as sendSignInLinkToEmail, f as OAuthProvider, ft as setPersistence, gt as signInWithEmailAndPassword, ht as signInWithCustomToken, j as confirmPasswordReset, k as browserSessionPersistence, l as GithubAuthProvider, lt as sendEmailVerification, o as EmailAuthProvider, pt as signInAnonymously, q as isSignInWithEmailLink, s as FacebookAuthProvider, st as reload, u as GoogleAuthProvider, ut as sendPasswordResetEmail, v as RecaptchaVerifier, vt as signInWithPhoneNumber, w as applyActionCode, wt as updateEmail, yt as signInWithPopup } from "./index.esm-BiaLojWn.js";
//#region node_modules/@capacitor-firebase/authentication/dist/esm/web.js
var FirebaseAuthenticationWeb = class FirebaseAuthenticationWeb extends WebPlugin {
	constructor() {
		super();
	...
	async applyActionCode(options) {
		return applyActionCode(getAuth(), options.oobCode);
	}
	async createUserWithEmailAndPassword(options) {
		const userCredential = await createUserWithEmailAndPassword(getAuth(), options.email, options.password);
		return this.createSignInResult(userCredential, null);
	}
	async confirmPasswordReset(options) {
		return confirmPasswordReset(getAuth(), options.oobCode, options.newPassword);
	}
	async confirmVerificationCode(options) {
		...
		if (!confirmationResult) throw new Error(FirebaseAuthenticationWeb.ERROR_CONFIRMATION_RESULT_MISSING);
		const userCredential = await confirmationResult.confirm(verificationCode);
		return this.createSignInResult(userCredential, null);
	}
	async deleteUser() {
		const currentUser = getAuth().currentUser;
		if (!currentUser) throw new Error(FirebaseAuthenticationWeb.ERROR_NO_USER_SIGNED_IN);
		return deleteUser(currentUser);
	}
	async fetchSignInMethodsForEmail(options) {
		return { signInMethods: await fetchSignInMethodsForEmail(getAuth(), options.email) };
	}
	...
	async getCurrentUser() {
		const auth = getAuth();
		return { user: this.createUserResult(auth.currentUser) };
	}
	async getIdToken(options) {
		const auth = getAuth();
		if (!auth.currentUser) throw new Error(FirebaseAuthenticationWeb.ERROR_NO_USER_SIGNED_IN);
		return { token: await auth.currentUser.getIdToken(options === null || options === void 0 ? void 0 : options.forceRefresh) || "" };
	}
	async getIdTokenResult(options) {
		...
		return Object.assign(Object.assign({}, idTokenResult), {
	...
	async getTenantId() {
		return { tenantId: getAuth().tenantId };
	}
	async isSignInWithEmailLink(options) {
		return { isSignInWithEmailLink: isSignInWithEmailLink(getAuth(), options.emailLink) };
	}
	...
	async linkWithPhoneNumber(options) {
		const currentUser = getAuth().currentUser;
		if (!currentUser) throw new Error(FirebaseAuthenticationWeb.ERROR_NO_USER_SIGNED_IN);
		if (!options.phoneNumber) throw new Error(FirebaseAuthenticationWeb.ERROR_PHONE_NUMBER_MISSING);
		if (!options.recaptchaVerifier || !(options.recaptchaVerifier instanceof RecaptchaVerifier)) throw new Error(FirebaseAuthenticationWeb.ERROR_RECAPTCHA_VERIFIER_MISSING);
		try {
	...
	async revokeAccessToken(options) {
		return revokeAccessToken(getAuth(), options.token);
	}
	...
	async sendPasswordResetEmail(options) {
		return sendPasswordResetEmail(getAuth(), options.email, options.actionCodeSettings);
	}
	async sendSignInLinkToEmail(options) {
		return sendSignInLinkToEmail(getAuth(), options.email, options.actionCodeSettings);
	}
	...
	async signInWithCustomToken(options) {
		const userCredential = await signInWithCustomToken(getAuth(), options.token);
		return this.createSignInResult(userCredential, null);
	}
	async signInWithEmailAndPassword(options) {
		const userCredential = await signInWithEmailAndPassword(getAuth(), options.email, options.password);
		return this.createSignInResult(userCredential, null);
	}
	async signInWithEmailLink(options) {
		const userCredential = await signInWithEmailLink(getAuth(), options.email, options.emailLink);
		return this.createSignInResult(userCredential, null);
	}
	...
	async unlink(options) {
		...
		return { user: this.createUserResult(user) };
	}
	...
	async useEmulator(options) {
		...
		if (options.host.includes("://")) connectAuthEmulator(auth, `${options.host}:${port}`);
		else connectAuthEmulator(auth, `${scheme}://${options.host}:${port}`);
	}
	...
	applySignInOptions(options, provider) {
		...
		if (options.scopes) for (const scope of options.scopes) provider.addScope(scope);
	}
	signInWithPopupOrRedirect(provider, mode) {
		const auth = getAuth();
		if (mode === "redirect") return signInWithRedirect(auth, provider);
		else return signInWithPopup(auth, provider);
	}
	linkCurrentUserWithPopupOrRedirect(provider, mode) {
		const auth = getAuth();
		if (!auth.currentUser) throw new Error(FirebaseAuthenticationWeb.ERROR_NO_USER_SIGNED_IN);
		if (mode === "redirect") return linkWithRedirect(auth.currentUser, provider);
		else return linkWithPopup(auth.currentUser, provider);
	}
	...
	createSignInResult(userCredential, authCredential) {
		return {
			user: this.createUserResult((userCredential === null || userCredential === void 0 ? void 0 : userCredential.user) || null),
			credential: this.createCredentialResult(authCredential),
			additionalUserInfo: this.createAdditionalUserInfoResult(userCredential)
		};
	}
	createCredentialResult(credential) {
		if (!credential) return null;
		const result = { providerId: credential.providerId };
	...
	createUserMetadataResult(metadata) {
		const result = {};
		if (metadata.creationTime) result.creationTime = Date.parse(metadata.creationTime);
		if (metadata.lastSignInTime) result.lastSignInTime = Date.parse(metadata.lastSignInTime);
		return result;
	}
	...
	createAdditionalUserInfoResult(credential) {
		...
		if (username !== null && username !== void 0) result.username = username;
		return result;
	}
	getErrorMessage(error) {
		if (error instanceof Object && "message" in error && typeof error["message"] === "string") return error["message"];
...
//# sourceMappingURL=web-CMGsySxa.js.map